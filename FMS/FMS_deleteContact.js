...
export const FMS_deleteContact = (token, recordId) => {
  const dynamicBody = "";
  const dynamicUrl =
    "https://" +
    FMS_HOST +
  ...
  console.log("FMS_deleteContact dynamicUrl: ", dynamicUrl);
  ...
  return fetch(dynamicUrl, {
    ...
    .catch((error) => {
      ...
      console.log(
        "FMS_deleteContact catch errorBlockString: ",
        errorBlockString
      );
      ...
      return new Promise((resolve, reject) => {
        ...
        if (errorBlockString.includes(`${subString}`)) {
          const composedError = {
            messages: [
              { code: 502, message: "FMS_deleteContact catch, 502 error" },
            ],
          };
          ...
          console.log(
            "FMS_deleteContact catch return Promise errorBlock != {}, errorBlock: ",
            errorBlock
          );
          ...
          console.log(
            "FMS_deleteContact catch return Promise errorBlock == {}"
          );
          const composedError = {
            messages: [
              {
                code: -99,
                message: "FMS_deleteContact catch, no errorBlock",
              },
            ],
          };
          reject(JSON.stringify(composedError));
