...
export const FMS_saveContact = (token, contact) => {
  const dynamicBody = {
    fieldData: {
  ...
  const dynamicUrl =
    ...
    FMS_LAYOUT_CONTACTS +
    "/records/" +
    contact.fieldData["Get|RecordID|"];

  console.log("FMS_saveContact dynamicUrl: ", dynamicUrl);
  console.log("FMS_saveContact dynamicBody: ", JSON.stringify(dynamicBody));
  ...
  return fetch(dynamicUrl, {
    ...
    .catch((error) => {
      ...
      console.log("FMS_saveContact catch errorBlockString: ", errorBlockString);
      ...
      return new Promise((resolve, reject) => {
        ...
        if (errorBlockString.includes(`${subString}`)) {
          const composedError = {
            messages: [
              { code: 502, message: "FMS_saveContact catch, 502 error" },
            ],
          };
          ...
          console.log(
            "FMS_saveContact catch return Promise errorBlock != {}, errorBlock: ",
            errorBlock
          );
          ...
          console.log("FMS_saveContact catch return Promise errorBlock == {}");
          const composedError = {
            messages: [
              {
                code: -99,
                message: "FMS_saveContact catch, no errorBlock",
              },
            ],
          };
          reject(JSON.stringify(composedError));
